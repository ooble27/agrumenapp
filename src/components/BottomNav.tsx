        ...
        {navItems.map((item) => {
          ...
          const content = (
            <div className="flex flex-col items-center gap-0.5 px-4 py-1.5 min-w-[64px] relative">
              <span
                className={`material-symbols-outlined text-[22px] transition-all duration-300 ${active ? "text-foreground scale-110" : "text-on-surface-variant"}`}
              ...
              <span className={`text-[10px] font-medium transition-colors duration-300 ${active ? "text-foreground font-bold" : "text-on-surface-variant"}`}>
                {item.label}
              </span>
              {active && (
                <motion.div
